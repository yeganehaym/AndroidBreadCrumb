 ...
 */
public class AndBreadCumb {

    //=-=--=-=-=-=-=-=-=-=-=-=-=- Private Properties -=-=-=-=-=-=-=--=-=-=
    private List<AndBreadCumbItem> items=null;
    ...
    public AndBreadCumb(Context context)
    {
    ...
    public void AddNewItem(AndBreadCumbItem item)
    {
        if(items==null)
            items=new ArrayList<>();
        items.add(item);
...
//we need to get size of layout,so we use the post method to run this thread when ui is ready
        layout.post(new Runnable() {
            ...
            public void run() {
                ...
                for (AndBreadCumbItem item : items) {
